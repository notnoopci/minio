...
func (r *Signature) DoesSignatureMatch(hashedPayload string) (bool, *probe.Error) {
	...
	switch {
	case contentSha == "" || contentSha == "UNSIGNED-PAYLOAD":
		// Unsigned Request - skip signature verification
		return true, nil
	case contentSha == "STREAMING-AWS4-HMAC-SHA256-PAYLOAD":
		break
	case contentSha != hashedPayload:
		return false, nil
	}
